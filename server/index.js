...
app.get('/api/qst/board', requireDeviceKey, async (req, res) => {
  try {
...
app.post('/api/qst/signals', requireDeviceKey, async (req, res) => {
  try {
    ...
    const { rows: countRows } = await pool.query(
      'select count(*)::int as count from qst_signals where user_id = $1',
      [userId]
    );
...
app.patch('/api/qst/signals/:id', requireDeviceKey, async (req, res) => {
  try {
    const { id } = req.params;
...
app.delete('/api/qst/signals/:id', requireDeviceKey, async (req, res) => {
  try {
    const { id } = req.params;
    const userId = await getUserId(req.deviceKey);
    const { rowCount } = await pool.query('delete from qst_signals where id = $1 and user_id = $2', [id, userId]);
...
app.get('/api/qst/signals/:id/rule', requireDeviceKey, async (req, res) => {
  try {
    const { id } = req.params;
    const userId = await getUserId(req.deviceKey);
    const { rows } = await pool.query(
...
app.post('/api/qst/signals/:id/rule', requireDeviceKey, async (req, res) => {
  try {
    const { id } = req.params;
...
app.delete('/api/qst/signals/:id/rule', requireDeviceKey, async (req, res) => {
  try {
    const { id } = req.params;
    const userId = await getUserId(req.deviceKey);
    await pool.query('delete from qst_rules where user_id = $1 and signal_id = $2', [userId, id]);
    res.json({ ok: true });
  } catch (err) {
    res.status(500).json({ error: String(err) });
  }
